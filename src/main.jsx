...
function AdminPanel({products,setProducts,settings,setSettings,onClose}){
 ...
 async function save(){if(!draft?.name)return;setBusy('Salvando produto...');setNotice('');try{const id=await saveProduct(draft,pendingFiles);if(draft.image&&/^https?:/.test(draft.image))await setProductCover(id,draft.image);await refresh();setDraft(null);setPendingFiles([]);setNotice('Produto salvo.')}catch(e){setNotice(e.message)}finally{setBusy('')}}
 ...
 async function fixedFile(e){const f=e.target.files?.[0];if(!f||!fixedKey)return;setBusy('Enviando imagem...');try{const url=await replaceSiteImage(fixedKey,f);setSettings(prev=>({...prev,siteImages:{...prev.siteImages,[fixedKey]:url}}));setNotice('Imagem atualizada.')}catch(err){setNotice(err.message)}finally{setBusy('');e.target.value='';setFixedKey(null)}}
 function chooseFixed(k){setFixedKey(k);setTimeout(()=>fixedRef.current?.click(),0)}
 async function remove(id){if(!confirm('Excluir este produto?'))return;setBusy('Excluindo...');try{await deleteProduct(id);await refresh();setNotice('Produto excluído.')}catch(e){setNotice(e.message)}finally{setBusy('')}}
 ...
 return <div className="admin-overlay"><aside className="admin-panel"><div className="admin-head"><div><img src={A+'logo-mark-navy.png'} alt="Lux Decor"/><div><b>Painel Lux Decor</b><small>Supabase exclusivo · Vercel seguro</small></div></div><div className="admin-head-actions"><button onClick={logout}>Sair</button><button onClick={onClose}>×</button></div></div>
 {notice&&<div className="admin-notice">{notice}</div>}{busy&&<div className="admin-busy">{busy}</div>}
 {!supabaseConfigured&&<div className="admin-setup-warning"><b>Variáveis públicas do Supabase ausentes.</b><span>Configure VITE_SUPABASE_URL e VITE_SUPABASE_PUBLISHABLE_KEY no Vercel e faça um novo deploy.</span></div>}
